...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 부트스트랩 모달 객체를 생성.
    const commentModal = new bootstrap.Modal('div#commentModal', { backdrop: true}); // 백그라운드를 검게 만들어주는 기능
...
//        const data = { postId:postId, username:username, ctext:ctext };
    ...
    function getAllComments(){
        ...
        const uri = `../api/comment/all/${postId}`; // 요청보낼 uri를 만든다. spring/api/
        ...
        function showCommentModal(event){
            ...
            .then((response) => {
                ...
                modalCommentId.value = comment.id;
                modalCommentText.value = comment.ctext;
        ...
        function updateComment() {
            ...
            const modalCommentId = document.querySelector('input#modalCommentId').value;
            const modalCommentText = document.querySelector('textarea#modalCommentText').value;
    
            const data = { modalCommentText } // 자바스크립트 객체를 만듦.
            console.log(data);
            const uri = `../api/comment/${modalCommentId}`
            axios
                .put(uri, data)
                .then((response) => {
                    console.log(response);
